...
- `ingest_pdf_to_private_knowledge_base`'s confirmation is an MCP elicitation (see [gofastmcp.com/servers/elicitation](https://gofastmcp.com/servers/elicitation)), which only works against a client that implements the elicitation capability; against one that doesn't, the tool catches the resulting protocol error and reports it as `action: "unsupported"` rather than writing unconfirmed or raising
